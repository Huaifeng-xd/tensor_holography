...
class TensorHolographyModel():
    ...
    def _build_model_vars(self):
        ...
        fnum = np.append(
            np.full((self.model_params["num_layers"] - 1), self.model_params["num_filters_per_layer"], dtype=int),
        ...
        for i in range(self.model_params["num_layers"]):
            ...
            # last layer
                ...
                in_dim, out_dim = fnum[i - 1] + self.model_params["input_dim"] * (
                            self.model_params["interleave_rate"] ** 2), \
                                  self.model_params["output_dim"] * (self.model_params["interleave_rate"] ** 2)
            else:
            ...
            model_vars[i] = {'weights': tf_init_weights([fw[i], fw[i], in_dim, out_dim],
                             ...
                             'bias': tf.Variable(
                                 tf.random.truncated_normal([out_dim], stddev=self.model_params["bias_stddev"]))
                             }
            ...
            if not i == self.model_params["num_layers"] - 1:
                layers[i] = self.model_params["activation_func"](
                    tf.layers.batch_normalization(
                        tf.nn.bias_add(
                            tf.nn.conv2d(prev_layers[i], model_vars[i]['weights'], strides=[1, 1, 1, 1], padding='SAME',
                ...
                if data_format == 'NCHW':
                    amp = tf.add(field[:, :self.model_params["output_dim"] // 2, :, :] * np.sqrt(0.5), np.sqrt(0.5),
                                 name="output_field_amp")
                    phs = tf.add(field[:, self.model_params["output_dim"] // 2:, :, :] * 0.5, 0.5,
                                 name="output_field_phs")
                else:
                    amp = tf.strided_slice(field, [0, 0, 0, 0], [1, field.shape[1], field.shape[2], 3], [1, 1, 1, 1],
                                           shrink_axis_mask=1)
                    phs = tf.strided_slice(field, [0, 0, 0, 3], [1, field.shape[1], field.shape[2], 6], [1, 1, 1, 1],
                                           shrink_axis_mask=1)
    ...
    def _setup_train(self):
        ...
        iterator = tf.compat.v1.data.Iterator.from_string_handle(handle,
                                                                 output_types=tf.compat.v1.data.get_output_types(
                                                                     train_iterator))
    ...
    def _get_loss(self,
        ...
        def get_depth_dependent_weight(depth, depth_to_focus, depth_diff_max):
            depth_diff = (depth_diff_max - tf.abs(depth - depth_to_focus)) * self.training_params[
                "depth_dependent_weight_scale"]
        ...
        for i in range(self.training_params["batch"]):
            ...
            idx = (tf.cast(tf.argsort(hist, direction="DESCENDING"), tf.float32) + tf.random.uniform([1], minval=0,
                                                                                                     maxval=1.0)) / \
                  self.training_params["num_hist_bins"]
            top_depth = idx[:self.training_params["num_top_depth_for_img_loss"]]
            rand_depth = tf.random.shuffle(idx[self.training_params["num_top_depth_for_img_loss"]:])[
                         :self.training_params["num_random_depth_for_img_loss"]]
            if not i:
        ...
        for i in range(self.training_params["batch"]):
            ...
            for j in range(depth_to_focus.shape[1]):
                tmp_img_loss, tmp_tv_loss, img_gt, img_out = get_img_diff_at_depth(y_out_slice, y_gt_slice,
                                                                                   depth_slice, depth_to_focus[i, j],
                                                                                   self.hologram_params["depth_scale"]
                                                                                   )
                fs_loss += tmp_img_loss
                fs_tv_loss += tmp_tv_loss
                ssim_img_loss += tf.reduce_mean(
                    tf.image.ssim(tf.transpose(img_gt, [0, 2, 3, 1]), tf.transpose(img_out, [0, 2, 3, 1]), 1.0))
                psnr_img_loss += tf.reduce_mean(
                    tf.image.psnr(tf.transpose(img_gt, [0, 2, 3, 1]), tf.transpose(img_out, [0, 2, 3, 1]), 1.0))
        ...
        ssim_amp_loss = tf.reduce_mean(
            tf.image.ssim(tf.transpose(y_gt_amp, [0, 2, 3, 1]), tf.transpose(y_out_amp, [0, 2, 3, 1]), 1.0))
        psnr_amp_loss = tf.reduce_mean(
            tf.image.psnr(tf.transpose(y_gt_amp, [0, 2, 3, 1]), tf.transpose(y_out_amp, [0, 2, 3, 1]), 1.0))
    ...
    def train(self):
        ...
        if self.training_params["restore_trained_model"]:
            ...
            if ckpt and ckpt.model_checkpoint_path:
                ...
                last_epoch = self.sess.run(global_step) / (
                            self.train_dataset_params["sample_count"] / self.training_params["batch"])
    ...
    def validate(self):
        ...
        ssim_amp_loss = tf.reduce_mean(
            tf.image.ssim(tf.transpose(amp_out, [0, 2, 3, 1]), tf.transpose(amp_in, [0, 2, 3, 1]), 1.0))
        psnr_amp_loss = tf.reduce_mean(
            tf.image.psnr(tf.transpose(amp_out, [0, 2, 3, 1]), tf.transpose(amp_in, [0, 2, 3, 1]), 1.0))
        ...
        print("validation results: ssim amp loss = %f(%f; %f/%f), pnsr amp loss = %f(%f; %f/%f)\n" % \
              (np.mean(avg_ssim_amp_loss), np.std(avg_ssim_amp_loss), np.amax(avg_ssim_amp_loss),
               np.amin(avg_ssim_amp_loss),
               np.mean(avg_psnr_amp_loss), np.std(avg_psnr_amp_loss), np.amax(avg_psnr_amp_loss),
               np.amin(avg_psnr_amp_loss)))

    def evaluate(self, eval_params):
        # define placeholder for input rgbd images
        rgbd = tf.compat.v1.placeholder(tf.float32,
                                        [1, self.model_params["input_dim"], eval_params['res_h'], eval_params['res_w']])
        ...
        rgb = np.transpose(cv2.resize(cv2.imread(eval_params['rgb_path']),
                                      dsize=(eval_params['res_w'], eval_params['res_h']),
                                      interpolation=cv2.INTER_CUBIC), [2, 0, 1])[:3, :, :] / 255.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval-rgb-path', default=os.path.join(cur_dir, "data", "example_input", "couch_rgb.png"),
                        help='Input rgb image path in evaluation mode')
    parser.add_argument('--eval-depth-path', default=os.path.join(cur_dir, "data", "example_input", "couch_depth.png"),
                        help='Input depth image path in evaluation mode')
    parser.add_argument('--eval-output-path', default=os.path.join(cur_dir, "data", "example_input"),
                        help='Output directory for results')
    parser.add_argument('--eval-depth-shift', default=0, type=float,
                        help='Depth shift (in mm) from the predicted midpoint hologram to the target hologram plane')
    ...
    # train parameters，
    parser.add_argument('--train-mode', action='store_true', default=True, help='Run in train mode')
    ...
    checkpoint_base_path = os.path.join(cur_dir, 'model', "ckpt_%s_pitch_%d_layers_%d_filters_%d" %
                                        (model_params["name"], hologram_params["pitch"] * 1000,
                                         model_params["num_layers"], model_params["num_filters_per_layer"]))
    path_params = {
        ...
        "validate_source_paths": [os.path.join(validate_base_path, x) for x in labels],
        # path to validation set raw images
